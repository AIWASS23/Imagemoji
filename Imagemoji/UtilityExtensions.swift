...
extension CGSize: RawRepresentable {
    ...
    public init?(rawValue: String) {
        ...
        if values.count == 2, let width = Double(values.first!), let heigth = Double(values.last!) {
            self.init(width: width, height: heigth)
        } else {
            return nil
        }
    }
...
extension Array where Element == NSItemProvider {
    ...
    func loadObjetcs<T>(ofType theType: T.Type, firstOnly: Bool = false, using load: @escaping (T) -> Void) -> Bool where T: NSItemProviderReading {
    ...
    func loadObjetcs<T>(ofType theType: T.Type, firstOnly: Bool = false, using load: @escaping (T) -> Void) -> Bool where T: _ObjectiveCBridgeable, T._ObjectiveCType: NSItemProviderReading {
        if let provider = first(where: { $0.canLoadObject(ofClass: theType) }) {
    ...
    func loadFirstObjetcs<T>(ofType theType: T.Type, using load: @escaping (T) -> Void) -> Bool where T: NSItemProviderReading {
        loadObjetcs(ofType: theType, firstOnly: true, using: load)
    }
    func loadFirstObjetcs<T>(ofType theType: T.Type, using load: @escaping (T) -> Void) -> Bool where T: _ObjectiveCBridgeable, T._ObjectiveCType: NSItemProviderReading {
